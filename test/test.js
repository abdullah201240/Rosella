...
const TOTAL_DAYS = 28; // June has 30 days

for (let day = 1; day <= TOTAL_DAYS; day++) {
  const commitCount = Math.floor(Math.random() * (MAX_COMMITS_PER_DAY - MIN_COMMITS_PER_DAY + 1)) + MIN_COMMITS_PER_DAY;

  console.log(`\n📅 Feb ${day}, 2024 - ${commitCount} commits`);

  for (let commitNum = 1; commitNum <= commitCount; commitNum++) {
    const commitDate = new Date(`2024-02-${String(day).padStart(2, "0")}T${String(commitNum + 10).padStart(2, "0")}:00:00`);

    // Modify file to ensure change
    fs.writeFileSync(FILE_NAME, `Commit on Feb ${day}, 2024 - #${commitNum} at ${commitDate.toISOString()}`);
    ...
    try {
      ...
      // Run Git commands
      execSync("git add .");
      execSync(`${gitEnv} git commit -m "Commit #${commitNum} on Feb ${day}, 2024"`, { stdio: "inherit" });

      console.log(`✅ Commit #${commitNum} for Feb ${day} created.`);
    } catch (error) {
      console.error(`❌ Error on commit #${commitNum} for Feb ${day}:`, error.message);
      break;
    }
  }
}
